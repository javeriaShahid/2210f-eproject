                    ...
                    @foreach ($data['blog'] as $blog )
                        <tr>
                            ...
                            <td>
                                ...
                                $tagsId =  json_decode($blog->tags);
                                $tags   = \App\Models\Category::whereIn('id' ,$tagsId)->paginate(2);
                                @endphp
                                @foreach ($tags as $tag )
                            ...
                            <td><label class="switch">
                                ...
                                <input type="checkbox" @if($blog->status == "0" ) @else checked @endif name="status" data-value = "{{$blog->id}}">
...
<script>
   ...
   let changeStatusUrl = "{{route('admin.blogs.change_status')}}" ;
    ...
    $.ajax({
        ...
        success:function(response){
            if(response.status == true ){
                e.preventDefault()
                toastr['success']("Blog has been Activated");
            ...
            if(response.status == "required"){
                e.preventDefault()
                toastr['error']("Failed to Change Status You Must Active Atleast 1 blog Information");
